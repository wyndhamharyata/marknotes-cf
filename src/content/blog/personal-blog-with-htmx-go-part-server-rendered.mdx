...
<u>**#Goldmark**</u> also brings its extension called **GFM (GitHub Flavored Markdown)** and
**Typographer**. We will need these two extensions to increase our markdown parsing capabilities.
...
<div class="p6 mx-auto bg-slate-100">{{.ParsedMarkdown}}</div>
{{end}}
```
...
<div id="markdown">
  <button hx-get="/markdown" hx-target="#markdown-part" hx-swap="innerHTML">Parse Markdown</button>
</div>
```
...
<BlogImage
  src="https://resource.mwyndham.dev/0-example-1-20240411112756.gif"
  alt="gif of renderring markdown"
/>

**Yeay!... or not?** While the Tailwind class we specified before is rendered correctly, the header looks the same... _what gives? Is the markdown rendering not working?_
...
<BlogImage
  src="https://resource.mwyndham.dev/0-0-a-20240409031752-20240421100132.webp"
  alt="Inspector on the Page to see if Markdown is renderred correctly"
/>

_But it looks a bit.._ _underwhelming_ isn't it? you would think that a well-rendered markdown would be similar to what GitHub offering would look like. But let alone have nice styling, `<h1>` and `<h2>` does not render any differently.
...
I won't go deep dive into what is Tailwind Typography, but I will just jump straight to the specific implementation we're aiming for: _stylize the different headers._
...
<div
  class="p6 prose prose-h1:text-4xl prose-h2:text-3xl prose-h3:text-2xl prose-h4:text-xl prose-h5:text-lg mx-auto bg-slate-100"
>
  {{.ParsedMarkdown}}
</div>
{{end}}
...
<html lang="en">
  ...
  <body>
    <div class="flex w-full flex-row">
      <textarea class="min-h-96 flex-1 p-6" id="editor"> </textarea>
      <div id="preview" class="min-h-96 flex-1 bg-stone-50 p-6"></div>
    </div>
  </body>
</html>
{{end}}
...
<BlogImage
  src="https://resource.mwyndham.dev/0-0-d-20240409032036-20240421100231.webp"
  alt="Split Editor"
/>

Next, we're going to repurpose the `markdown.html` we had previously to display the content on the preview side. But first, we need to modify it:

```html
<!-- before -->
<div
  class="prose prose-h1:text-4xl prose-h2:text-3xl prose-h3:text-2xl prose-h4:text-xl prose-h5:text-lg mx-auto p-6"
>
  <!-- after -->
  <div
    class="prose prose-h1:text-4xl prose-h2:text-3xl prose-h3:text-2xl prose-h4:text-xl prose-h5:text-lg"
  ></div>
</div>
```
...
<BlogImage
  src="https://resource.mwyndham.dev/0-0-f-20240409032043-20240421100300.webp"
  alt="Inspect on the Split Editor to se if htmx works"
/>

Which is good! now we know that we can interactively send the text area content to the server, and the server will be able to render the HTML for us!
...
<BlogImage src="https://resource.mwyndham.dev/0-output-20240311033039.gif" alt="" />

And just like that, we have our server-rendered Markdown done. It's not that much code but we achieve quite a lot here!
